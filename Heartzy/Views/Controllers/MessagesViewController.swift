...
//        view.layer.maskedCorners = [.layerMinXMaxYCorner,.layerMaxXMaxYCorner]
    ...
    lazy var Perimg : UIImageView = {
        let iv = UIImageView()
        iv.layer.cornerRadius = 30 * appConstant.heightRatio
...
//        tv.layer.cornerRadius = 18 * appConstant.heightRatio
        ...
        tv.keyboardDismissMode = .interactiveWithAccessory // or .interactive
...
//        view.layer.maskedCorners = [.layerMinXMaxYCorner,.layerMaxXMaxYCorner]
    ...
    var count = 0
    var totalMesgCount : Int = 0
    var arr = [0,1]
    var theData = [String : Any]()
    var chatId = String()
    
    var isCommingFromColorChange = false
...
//        AppUserDefault.shared.isAppOpenCount = "0"

        
        if  AppUserDefault.shared.isPayment == true
        {
        ...
        else
        {
            ...
            if AppUserDefault.shared.isLogin == false
            {
                
            }
            else
            {
                if AppUserDefault.shared.isAppOpenCount ?? "" <= "6"
                {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.4)
                    {
                        let vc = HalfPricingPlanController()
            ...
            if AppUserDefault.shared.isFirstTime == true
            {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 2.0){
                    ...
                    AppUserDefault.shared.isFirstTime = false
...
//        Check that if Guided Access is On or OFF
        if UIAccessibility.isGuidedAccessEnabled {
          // show something since I'm not in guided access
            print("The Guided Access is ON")
        }
        else
        {
            print("The Guided Access is OFF")
        }
...
//            isCommingFromSetting = true
        ...
        {
            LogoImg.isHidden = false
            LogoImg.image = UIImage(named: "DarkAcademia2")
            DataTableview.backgroundColor = .clear
        ...
        {
            LogoImg.isHidden = false
            LogoImg.image = UIImage(named: "NightSky2")
            DataTableview.backgroundColor = .clear
        ...
        else if AppUserDefault.shared.theme5 == true
        {
            ...
            LogoImg.isHidden = false
            LogoImg.image = UIImage(named: "Stars2")
            DataTableview.backgroundColor = .clear

        }
        else if AppUserDefault.shared.theme6 == true
        {
            
            LogoImg.isHidden = false
            LogoImg.image = UIImage(named: "Flowers2")
            DataTableview.backgroundColor = .clear
...
//        view.addSubview(bottomView)
        ...
        NSLayoutConstraint.activate([
            ...
            settingBtn.centerYAnchor.constraint(equalTo: headerView.centerYAnchor, constant: 12 * appConstant.heightRatio),
            ...
            Perimg.centerYAnchor.constraint(equalTo: headerView.centerYAnchor, constant: 12 * appConstant.heightRatio),
            Perimg.leadingAnchor.constraint(equalTo: headerView.leadingAnchor, constant: 20 * appConstant.widthRatio),
            Perimg.heightAnchor.constraint(equalToConstant: 60 * appConstant.heightRatio),
            Perimg.widthAnchor.constraint(equalToConstant: 60 * appConstant.heightRatio),
...
//            sendTextTF.widthAnchor.constraint(equalToConstant: 310 * appConstant.heightRatio),
            sendTextTF.heightAnchor.constraint(equalToConstant: 55 * appConstant.heightRatio),
            ...
            sendmsgBtn.heightAnchor.constraint(equalToConstant: 50 * appConstant.heightRatio),
            sendmsgBtn.widthAnchor.constraint(equalToConstant: 50 * appConstant.heightRatio),
            ...
            shadowView.heightAnchor.constraint(equalToConstant: 460 * appConstant.heightRatio),
...
//            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0){ [self] in
            ...
            if isEmoji(self.sendTextTF.text ?? "")
            {
                ...
                if (self.sendTextTF.text ?? "").containsOnlyEmoji
                {
                    let the = "\u{2764}\u{FE0F}" + "\u{1F9E1}\u{FE0F}"
                    
                    let the2 = "\u{1F499}\u{FE0F}" + "\u{1F49A}\u{FE0F}"
                    
                    let the3 = "\u{1F496}\u{FE0F}" + "\u{1F49C}\u{FE0F}"
                    
                    self.messages.append(message(senderid: "2", message: the.encodeChatString()! + (self.sendTextTF.text?.encodeChatString())!! + the2.encodeChatString()! + (self.sendTextTF.text?.encodeChatString())! + the3.encodeChatString()!, datetime: "01"))
                }
                else
                {
                    let the = "\u{2764}\u{FE0F}" + "\u{1F9E1}\u{FE0F}"
                    ...
                    var theSend = (self.sendTextTF.text)?.emojiString
                    
                    self.messages.append(message(senderid: "2", message: the.encodeChatString()! + (theSend?.encodeChatString()!)! + the2.encodeChatString()! + (theSend?.encodeChatString()!)! + the3.encodeChatString()!, datetime: "01"))
                }
            ...
            else
            {
                ...
                self.messages.append(message(senderid: "2", message: the.encodeChatString()! + theRandomEmoiji.encodeChatString()! + the2.encodeChatString()! + theRandomEmoiji.encodeChatString()! + the3.encodeChatString()!, datetime: "01"))
...
extension MessagesViewController :  UITableViewDelegate,UITableViewDataSource {
            ...
            else
            {
                if messages.count % 4 == 0
                {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 1.0){
                        ...
                        self.confView.particles = [.star, .arc, .triangle, .circle, .heart]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if data.senderid == "2"{
            ...
            {
                cell.recieverimg.image = UIImage(named: "Bot5")
            }
//            else {
//                cell.recieverimg.image = UIImage(named: "Bot1")
//            }
...
extension MessagesViewController : UITextViewDelegate{
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        print("The Total Count is ",textField.text?.count)
        
        totalMesgCount = textField.text?.count ?? 0
